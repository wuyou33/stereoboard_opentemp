                               ...
                               uint16_t feature_number_select, uint32_t image_width, uint32_t image_height, uint8_t feature_window_size_half)
{
  ...
  for (i = 0; i < feature_count; i++) {
    ...
    for (col = col_center - feature_window_size_half; col < col_center + feature_window_size_half + 1;
         col++) { // runthrough all images lines corrsponding to feature window
      for (row = row_center - feature_window_size_half; row < row_center + feature_window_size_half + 1;
           row++) { // runthrough all images lines corrsponding to feature window
    ...
    for (col = col_center_right - feature_window_size_half; col < col_center_right + feature_window_size_half + 1;
         col++) { // runthrough all images lines corrsponding to feature window
      for (row = row_center - feature_window_size_half; row < row_center + feature_window_size_half + 1;
           row++) { // runthrough all images lines corrsponding to feature window
    ...
    float32_t *rotation_coefficients, int number_of_rotations, uint32_t image_width)
{
  ...
  for (i = 0; i < features_ROT_number; i++) {
    ...
    arm_scale_f32(&rotation_coefficients[number_of_rotations * 0], (float32_t) feature_image_coordinates[index + 0], row,
                  (uint32_t) number_of_rotations); // x = A*x
    arm_scale_f32(&rotation_coefficients[number_of_rotations * 1], (float32_t) feature_image_coordinates[index + 1], temp,
                  (uint32_t) number_of_rotations); // temp = B*y

    arm_add_f32(&rotation_coefficients[number_of_rotations * 2], temp, temp,
                (uint32_t) number_of_rotations); // temp = (B*y) + C
    arm_add_f32(row, temp, row, (uint32_t) number_of_rotations);  // x = x + temp = (A*x) + (B*y) + C

    // compute translated y-coordinates
    arm_scale_f32(&rotation_coefficients[number_of_rotations * 3], (float32_t) feature_image_coordinates[index + 0], col,
                  (uint32_t) number_of_rotations); // col = D*x
    arm_scale_f32(&rotation_coefficients[number_of_rotations * 4], (float32_t) feature_image_coordinates[index + 1], temp,
                  (uint32_t) number_of_rotations); // temp = E*y

    arm_add_f32(&rotation_coefficients[number_of_rotations * 5], temp, temp,
                (uint32_t) number_of_rotations); // temp = (E*y) + F
    ...
    */
    ...
    arm_scale_f32(&rotation_coefficients[number_of_rotations * 8], (float32_t) feature_image_coordinates[index + 2], disp,
