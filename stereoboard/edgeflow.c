                   ...
                   int16_t image_width, int16_t image_height, int8_t use_monocam)
{
  ...
  edgeflow_parameters->stereo_shift =  -6;
  ...
  edgeflow_results->edge_flow.flow_x =
    edgeflow_results->prev_edge_flow.flow_x = 0;
  edgeflow_results->edge_flow.div_x = edgeflow_results->prev_edge_flow.div_x =
                                        0;
  edgeflow_results->edge_flow.flow_y =
    edgeflow_results->prev_edge_flow.flow_y = 0;
  edgeflow_results->edge_flow.div_y = edgeflow_results->prev_edge_flow.div_y =
                                        0;
...
//int32_t *prev_vel_y = &edgeflow_results->prev_vel_y;
  ...
  *hz_y = TIMER_TICKS_PER_SEC
  ...
  for (x = 0; x < IMAGE_WIDTH - 1; x++) {
    ...
    {
      edgeflow_results->stereo_distance_per_column[x] = RES * RES * 6
          * 128
          / abs(
            edgeflow_results->displacement.stereo[x]
                         ...
                         struct edgeflow_results_t *edgeflow_results)
{
  ...
  *avg_disp = calculate_displacement_fullimage(edge_histogram_x,
  ...
  // Fit a linear line
  //FIXME flow as measured from middle of image
                                       ...
                                       uint8_t window, uint8_t disp_range, int16_t stereo_shift)
{
  ...
  if (stereo_shift > 0) {
    border_left = W + D + stereo_shift;
  } else {
    if (stereo_shift < 0) {
      border_right = size - W - stereo_shift;
    }
  }
    ...
    int32_t *edge_histogram_2, uint16_t size, uint8_t disp_range, int16_t stereo_shift)
{
  ...
  if (stereo_shift > 0) {
    border_left =  D + stereo_shift;
  } else {
    if (stereo_shift < 0) {
      border_right = size - stereo_shift;
    }
  }
  ...
  for (c = -D; c <= 0; c++) {
    SAD_temp[c + D] = 0;
    for (x = border_left; x < (int32_t) size - D; x++) {
      SAD_temp[c + D] += abs(edge_histogram[x] - edge_histogram_2[x + c - stereo_shift]);
    }
  }
                                ...
                                int32_t *vel_z_stereo_avoid_pixelwise, int32_t max_velocity, int32_t size, int32_t border, int16_t stereo_shift)
{
  ...
  *vel_x_stereo_avoid_pixelwise = vel_x_temp / (total_size * 2 / 3);
  *vel_z_stereo_avoid_pixelwise = vel_z_temp / (total_size  / 3);

}


// Line_fit fits a line using least squares to the histogram disparity map
//TODO: weighted Linefit with uncertainty
...
void totalKalmanFilter(struct covariance_t *coveriance,
                       struct edge_flow_t *prev_edge_flow, struct edge_flow_t *edge_flow,
                       uint32_t Q, uint32_t R, uint32_t RES)
{
  edge_flow->flow_x = simpleKalmanFilter(&(coveriance->C_flow_x),
                                         prev_edge_flow->flow_x, edge_flow->flow_x, Q, R, RES);
  edge_flow->flow_y = simpleKalmanFilter(&(coveriance->C_flow_y),
                                         prev_edge_flow->flow_y, edge_flow->flow_y, Q, R, RES);
  edge_flow->div_x = simpleKalmanFilter(&(coveriance->C_div_x),
                                        prev_edge_flow->div_x, edge_flow->div_x, Q, R, RES);
  edge_flow->div_y = simpleKalmanFilter(&(coveriance->C_div_y),
                                        prev_edge_flow->div_y, edge_flow->div_y, Q, R, RES);
}
